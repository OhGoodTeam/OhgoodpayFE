...
const USE_MOCK = import.meta.env.DEV && import.meta.env.VITE_USE_MOCK_SPENDING === "true"
const ANALYZE_URL = "/api/dash/analyze";
...
export const useSpendingAnalysisStore = create((set, get) => ({
  ...
  async fetchSpendingData(customerId = 1, monthsCount = 3) {
    // set({ loading: true, error: null });
     let run = true;
     set((s) => (s.loading ? ((run = false), s) : { loading: true, error: null }));
     if (!run) return;
    try {
      let payload;
      ...
      if (USE_MOCK) {
        ...
        if (!res.ok) throw new Error(json?.message || `API 호출 실패: ${res.status}`);
