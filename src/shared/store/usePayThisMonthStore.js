...
const normalize = (json) => {
  ...
  // 합계 금액
  const amountRaw =
    d.sumTotalPrice ?? d.amount ?? d.bnplAmount ?? d.bnpl_this_month_amount ?? 0;

  // 항목
  const rows = d.items ?? d.transactions ?? d.recentTransactions ?? d.installments ?? [];
  ...
  const pointsThisMonthRaw =
    d.sumPointThisMonth ??
    d.points ?? d.earnedPoints ?? d.pointEarnedThisMonth ??
...
const useBNPLStore = create((set) => ({
  ...
  fetchThisMonth: async (customerId = 1) => {
    ...
    try {
      ...
      // });
      const res = await fetch(`/api/dash/customer/${encodeURIComponent(customerId)}/pay-this-month`, {
        method: "GET",
